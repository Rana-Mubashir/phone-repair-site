          ...
          <div className="flex flex-col-reverse md:flex-row items-center justify-between">
            <div className="md:w-1/2 mt-10 md:mt-0">
              <div className="relative inline-block mb-3">
                <h1 className="text-4xl md:text-5xl font-bold text-[#34c5f1]">Device Categories</h1>
                <div className="absolute -bottom-3 left-0 w-24 h-1 bg-[#34c5f1] rounded-full"></div>
              </div>
              ...
              <div className="mt-8 space-y-4">
                <div className="flex items-center gap-3">
                  <div className="bg-[#34c5f1]/10 p-2 rounded-full">
            ...
            <div className="md:w-1/2 flex justify-center">
              <div className="relative">
                <div className="absolute -inset-4 bg-gradient-to-r from-white to-blue-50 rounded-full blur-lg opacity-70 animate-pulse"></div>
                <img
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-gray-800 inline-block relative">
            Select Your Device
            <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-20 h-1 bg-[#34c5f1] rounded-full"></div>
          </h2>
        ...
        {loading ? (
          <div className="flex justify-center items-center py-20">
            <div className="flex flex-col items-center">
              <div className="relative">
                <div className="absolute inset-0 bg-gradient-to-r from-[#34c5f1]/20 to-blue-300/20 rounded-full blur opacity-70"></div>
                <Loader2 className="h-12 w-12 text-[#34c5f1] animate-spin relative" />
              </div>
          ...
          <div className="flex justify-center py-10">
            <div className="bg-red-50 border border-red-100 rounded-xl p-8 max-w-md text-center shadow-sm">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-[#34c5f1] text-white rounded-lg hover:bg-[#2ba8d0] transition-colors shadow-sm"
              >
                Try Again
              </button>
            </div>
          ...
          <>
            {data && data.length > 0 ? (
                ...
                <div className="flex justify-center items-center flex-nowrap gap-6 min-w-full pb-4">
                  {data.map((category, index) => (
                    <Link href={`/select-brand/${category._id}`} key={index} className="w-64 flex-shrink-0 first:ml-0">
                      <div className="bg-white rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 h-full flex flex-col overflow-hidden group border border-blue-50">
                        <div className="p-6 flex items-center justify-center bg-gradient-to-br from-blue-50 to-white flex-grow">
                          {category?.image ? (
                            <img
                            ...
                            <div className="h-48 w-48 flex items-center justify-center bg-blue-50 rounded-full">
                              <Smartphone className="h-20 w-20 text-[#34c5f1]/50" />
                            </div>
                          )}
                        </div>
                        <div className="p-4 border-t border-blue-50 flex items-center justify-between bg-white">
                          <h3 className="font-medium text-gray-800">{category?.name}</h3>
                          <div className="bg-[#34c5f1]/10 p-1.5 rounded-full transform translate-x-0 group-hover:translate-x-1 transition-transform">
                            <ArrowRight className="h-4 w-4 text-[#34c5f1]" />
                          </div>
                        </div>
                      </div>
                    </Link>
                ...
                {/* <div className="flex justify-center mt-6">
                  <div className="flex gap-1.5">
                    {[...Array(Math.min(5, Math.ceil(data.length / 4)))].map((_, i) => (
                      <div
                        key={i}
                        className={`h-2 w-2 rounded-full ${i === 0 ? "bg-[#34c5f1]" : "bg-[#34c5f1]/30"}`}
                      ></div>
                    ))}
                  </div>
                </div> */}
              ...
              <div className="py-16 flex justify-center">
                <div className="bg-white rounded-xl p-10 shadow-sm border border-blue-50 max-w-md text-center">
                  <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-blue-50 mb-4">
                    <Smartphone className="h-8 w-8 text-[#34c5f1]/50" />
                  </div>
      ...
      {data && data.length > 0 && (
        <div className="container mx-auto px-4 py-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white rounded-xl p-6 shadow-sm border border-blue-50 hover:shadow-md transition-all duration-300">
              <div className="p-3 rounded-full bg-[#34c5f1]/10 w-fit mb-4">
                <svg
